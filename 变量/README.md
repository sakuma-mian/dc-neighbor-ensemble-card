...
## 3. 本卡变量语义决策（DEC-025 驱动裁决 + DEC-033 增补，2026-08-29）
...
- **好感度基准 = 0（线上好友）**——DEC-033 驱动裁定取代 DEC-025 的"基准 50"；范围仍 -100~100
- **关系阶段十二档阶梯（DEC-033）**：由好感度唯一推导（写入值不得偏离区间）：100 永不背叛／90~99 交换誓言／70~89 红颜知己／50~69 挚爱亲朋／40~49 无话不谈／20~39 相知相识／-19~19 线上好友（0 基准）／-20~-39 关系僵硬／-40~-49 互不顺眼／-50~-69 不相往来／-70~-89 互为仇敌／-90~-99 不共戴天／-100 不死不休
- **身份揭晓 = 关系行新增布尔字段（DEC-033，默认 false）**：主控知晓该角色线下真实身份的标记；仅正文明确揭开场景可置 true，不可逆。**好感度门禁**：未揭晓时好感度必须在 -49~+49 内（进不了 挚爱亲朋/不相往来 档）；越界必须在同轮次揭晓身份
- 稳定键：主控 / 花奈 / 清寒 / 沉秋 / 楚羽笙 / 诺薇拉；网名、化名变化不改键（楚羽笙 换掉 _unofficial 的终局不改键）
- 任务表当前为空（DEC-203），更新规则已禁止凭空新增；变量层 position/order 仍留组装门核定
- 前端消费：`frontend/排版设计/变量映射-v1.md`（djg-render.js 只读 stat_data；揭晓态真名显示由 身份揭晓 字段驱动）
...
2. initvar 与 schema 默认值一致（好感度 0、关系阶段 线上好友、身份揭晓 false、迷雾全未揭示、登场全 false、初始化完成 false、schema版本 dc-neighbor-state-v1）
