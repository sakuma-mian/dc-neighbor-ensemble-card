...
updated: 2026-08-29
next_gate: ACC-006 真实 SillyTavern 运行时验收（驱动另约：开局初始化→变量更新→门控生效→渲染链）→ 于此之前开放项：DEC-027 三档位与防情感廉价、DEC-036 背景方案（天际线）追认；DEC-202 城区修订 / DEC-203 任务层 / DEC-204 彩蛋层 / DEC-205 备选开场与起源层创始人补齐后重跑 组装/build_card.mjs 即可重建卡
---
...
- [DEC-028] **前端设计线开工（雾青排版系统 v0.1）**：驱动四项裁定——① 保留原神官网**排版骨架**（左像右栏角色页/居中章节标题/日期+标题列表行/半透明面板+模糊），但配色与质感必须与原神官网**明显区别**，现阶段**只做排版设计**；② 酒馆状态栏与独立预览页**都要**；③ 迷雾双态图标可用已有素材**拼接**但不得直接套用神之眼 UI（落地方案：镜片素材去饱和封雾＋六边形细线框，无金翼框、无元素纹章）；④ **全部前端资源仅存本地、不入公开仓库**（.gitignore 已显式排除 `frontend/`）。产出：`frontend/排版设计/排版系统-v0.1.md`（雾青 token：底 #0B1014／雾青 #8FD0D8／灰靛 #6C7FA8／切角面板／等宽数字，附与官网区分点验收清单）＋ `preview.html`（群像五卡／单人角色页／时间线／关系网·五人十对占位排版，浏览器实开校验并修复 frame 拉伸露底缺陷）＋ `状态栏-v0.1.html`（自包含 `djg-` 前缀组件，纯 CSS 雾珠零外部依赖，三档演示含负好感陶红态）。素材底座＝原神官网抓取的 43 件 UI 资源（`frontend/assets/genshin-official/`，**© miHoYo 非开源**，仅限本地非商业同人使用，SOURCES.txt 全量溯源；MAT-014 登记待驱动追认）。边界：嵌入 UI 仍按 Exclusions 不进卡包 display_data，本线为本地设计稿，不接 MVU 变量、不动设定集内容。（2026-08-29，驱动四项裁定/agent 执行）

- [DEC-029] **前端设计线 v0.2（迷雾展示纪律 / 双头像位 / 群聊视图）**：驱动过目 v0.1 后三条新裁定——① **迷雾命名纪律**：状态栏与群聊视图雾态一律不出现角色真名（真名只存在于揭晓态：揭晓系统消息"≡"标记、档案揭晓字段、二号头像位）；② **双头像位制度**：一号位＝DC 头像（雾态默认展示），二号位＝角色立绘（身份揭晓后切换展示），前端仅需状态类切换（演示用 CSS `:checked`，实卡由 R2 门卫驱动）；③ **页面裁剪**：关系网页与时间线删除，仅保留**群像页（重构为群聊视图：频道头/消息流/引用块/成员栏/输入条，成员名全用网名+冷家族色）**与**单人角色页（加雾态/揭晓切换演示，雾态默认真名=？？？）**。产出已实开校验并修 kv 栅格错行 bug：`frontend/排版设计/`（preview.html + 状态栏组件 v0.2 + 排版系统文档 v0.2）。边界同 DEC-028：本地设计稿、占位数据、不接 MVU。（2026-08-29，驱动裁定/agent 执行）

- [DEC-030] **前端设计线 v0.3（仅变量纪律 / 群像页退为跳板）**：驱动过目 v0.2 群聊视图后裁定——① 群像界面**不出现非变量数据**；② **废除聊天页面**；③ **群像页仅为单人档案的跳板**。落地方案：新增 §0.5「仅变量纪律」白名单（网名/身份状态/好感度/关系阶段/迷雾格/环境时间/头像位状态）/黑名单（攻略难度、常出没、攻略轴、天气信号、语录、聊天记录、一切词库文本）；preview.html 群像节重构为五张成员卡跳板栅格（整卡锚点跳转单人档案，雾态卡与揭晓卡双态演示），单人档案剥离静态词条（常出没/攻略/信号/语录删除，基本档改关系档：网名/身份/关系阶段）；状态栏同步仅变量（删难度徽标/地点/天气信号，底行改 环境时间｜关系阶段）。校验中修两处布局缺陷：跳板卡长网名撑宽面板（flex 链缺 min-width:0）、基本档双态行栅格错位。**产出待决项移交变量层**：揭晓态真名要在前端渲染，需将真名字符串随揭晓写入变量（如 `关系.*.别名` 或迷雾揭晓附录）——随组装门核定。边界同 DEC-028/029：本地设计稿、占位数值、不接 MVU。（2026-08-29，驱动裁定/agent 执行）

- [DEC-031] **前端设计线 v0.4（一人一档案，跳板卡各自跳转）**：驱动裁定群像跳板卡应跳转到**该角色对应的档案界面**（此前五卡共用花奈演示页）。落地：五人各建一页（`#m1..#m5`，网名/头像/真名/阶段/迷雾格数按各角色占位数据），跳板卡锚点一一对应；页内切换用纯 CSS `:target`（无锚点默认显示第一人，无 JS）；每页独立雾态/揭晓切换（五组独立 radio id，联动头像位与身份字段）。实开校验：无锚点默认态、跨卡跳转（m3/m5）、逐页揭晓切换均通过 DOM 几何验证。边界同前：本地设计稿、占位数值、不接 MVU。（2026-08-29，驱动裁定/agent 执行）

- [DEC-032] **前端数据接线 v1（排版定稿 → MVU 真数据渲染）**：驱动确认雾青 v0.4 排版定稿并指令连接 MVU 变量。落地（`frontend/排版设计/`）：① **数据契约** `变量映射-v1.md`——UI↔stat_data 逐字段映射（迷雾 UI 视角=主控行、前缀→角色归属 H花奈/Q清寒/C沉秋/Y楚羽笙/N诺薇拉、好感条宽=(值+100)/2%、亮珠=已揭示/误信已纠正、环境时间=日期+时段十值枚举），附缺口清单（身份揭晓标记/真名字符串/关系阶段枚举/任务容器）与酒馆接线说明（实卡监听 `mag_variable_update_ended` 读 `Mvu.getMvuData().stat_data`，与演示页共享渲染代码）；② **渲染器** `djg-render.js`（零依赖，derive+barHTML/hubCardHTML/archiveHTML，只读 stat_data）；③ **schema 合法 mock** `mock-stat-data.json`（主控行 2 处翻转演示：H.家史已揭示、Q.霸凌误信已纠正）；④ **真数据演示页** `状态栏-live.html`/`preview-live.html`（fetch mock → 同一管线渲染五人状态栏与跳板+档案）。实开校验通过：五栏数值/条宽/珠态全对（含 误信已纠正 计亮珠）、翻转按钮 1/5→0/5 与总数 2/21→1/21 响应、揭晓按钮 djg-rv 切换（真名行/立绘字/竖排名联动）。渲染器对缺口①留 `DJG.setRevealed` 宿主注入接口；变量层增设揭晓标记后前端切换读变量。渲染器不写任何变量。（2026-08-29，驱动指令/agent 执行）

- [DEC-033] **变量层增补（关系行·身份揭晓 + 好感门禁 + 阶段十二档阶梯）**：驱动三条裁定——① 创建身份揭晓条件变量，达成即判定 网名=真名；② 真名显示随该变量出现在状态栏；③ 关系阶段阶梯以 0 为基准线（线上好友），且**未揭晓线下身份时好感度不得越过 +50 / 跌破 -50**。落地（变量层四处 + 前端同步）：**[initvar]初始**——关系行结构改为 {好感度, 关系阶段, 身份揭晓:false}，好感度默认 **0**（取代 DEC-025 的"基准 50"，依据本裁定"基准线为0"；基准线名称 线上群友→线上好友）；**酒馆助手脚本-ZOD.json**——relationEntry 增 身份揭晓 boolean（prefault false），好感 prefault 50→0；**[mvu_update]变量更新规则**——关系节重写：身份揭晓仅正文明确线下揭开场景可置 true 且不可逆；好感门禁（未揭晓 -49~+49，越界必须同轮揭晓）；阶段十二档阶梯表（100 永不背叛／90~99 交换誓言／70~89 红颜知己／50~69 挚爱亲朋／40~49 无话不谈／20~39 相知相识／-19~19 线上好友／-20~-39 关系僵硬／-40~-49 互不顺眼／-50~-69 不相往来／-70~-89 互为仇敌／-90~-99 不共戴天／-100 不死不休），写入阶段值不得偏离好感度区间；**变量/README** §3/§5 同步。前端：djg-render v1.1——身份揭晓读 `关系.<键>.身份揭晓`（原 revealedOverride 接口废除），阶段由好感度按阶梯推导（不信任存储值），门禁越界置 vm.门禁 标记；mock 数据改为门禁合规取值（花奈 52 已揭晓挚爱亲朋／清寒 -45 互不顺眼／沉秋 47／楚羽笙 22／诺薇拉 8）；状态栏组件参考页与 live 演示页同步。实开校验通过：五栏阶梯推导全对、揭晓按钮翻 `关系.*.身份揭晓` 即时切换 djg-rv+真名行、跳板卡揭晓态原生驱动。真名字符串仍在前端 CONFIG（显示由变量门控，裁定②落实；字符串变量化随组装门核定，非阻塞）。（2026-08-29，驱动三条裁定/agent 执行）

- [DEC-034] **五人头像与立绘·GPT 生图提示词落盘**：按用户指令从五份 v1.0 正式稿（MAT-007..012）提炼形象锚点，产出 `设定集/人设/头像立绘提示词-GPT生图.md`——每角色两段完整可贴提示词（**线上 DC 头像 + 线下真人立绘**，日系二次元风，GPT 自然语言长段式）+ 每人 2-3 个可选差分。关键设计：头像＝线上头像、立绘＝线下真人，与 DEC-029 双头像位对位（一号位=DC 头像雾态、二号位揭晓态可由立绘裁切）；四张头像是档案原文写死的"与本人零关联"意象（花奈风铃/清寒黑猫/沉秋奶牛猫/诺薇拉金发少女），楚羽笙头像按其假口径"142双马尾红瞳雌小鬼"提案；立绘锚点零编造（清寒绿发金瞳+创可贴+缠绷带棒球棍、沉秋 186cm 黑长直全身黑+降噪耳机+左耳单只乌鸦耳坠、诺薇拉细框眼镜+护身挂坠+微丰腴等均档案明示）。**4 处开放项标【待驱动定】**：沉秋/楚羽笙/诺薇拉瞳色（档案未写，暂填深灰蓝/浅紫灰/暖棕）、楚羽笙头像形象确认。迷雾纪律注记：游戏内显示只用头像图，立绘不可作为线上形象出现；成图入库按 MAT-014+ 登记溯源。（2026-08-29，用户指令/agent 执行）

- [DEC-035] **前端接入角色头像/立绘（命名映射 + 本地资产化 + git 排除）**：驱动提供 `设定集/头像/` 成图（10 文件 PNG，即 DEC-034 提示词的产出）。落地：① 本地拷贝至 `frontend/assets/头像/`（frontend/ 不入库，裁定④）；② **`.gitignore` 增补 `/设定集/头像/` 显式排除**——该目录位于已白名单的 设定集/ 内，不排除将随下次提交进入公开仓库，违反裁定④；③ djg-render CONFIG 增 头像图/立绘图 路径（按命名映射：网名→一号位头像、真名→二号位立绘），状态栏/跳板卡/档案页头像位嵌 img（object-fit cover、object-position 50% 15% 偏面部，加载失败回退首字字牌），档案大图雾态=一号位头像放大、揭晓=二号位立绘 contain 全身；④ 状态栏组件参考页与 live 演示页同步。迷雾纪律自洽：雾态 UI 只出现头像图，立绘仅在揭晓态出现（符合 DEC-034 注记）。实开校验：10 图全部可载（跳板 5/5、档案立绘 1024×1536、雾/揭晓换图正确）。成图 MAT 登记（DEC-034 预留的 MAT-014+ 槽位）与本线先前提议的 原神官网素材包 MAT 编号需一并排定，待驱动追认。**驱动报缺陷后修复**：档案大图 雾态头像层/揭晓立绘层 双层同显重叠——根因 `.portrait .pimg{display:flex}` 与通用显隐规则同权重且靠后生效；改 pimg 专属互斥规则；连带弃用 `:target`（innerHTML 重渲染后 Chromium 丢失 target 关联致视图弹回 m1），改 hashchange 监听 + `.arch-on` 类驱动；同报修第二项：档案大图中央叠层文字（雾态网名/揭晓真名竖排+说明注记）压图——已整体移除，大图区域只渲染图像（名字仍在节标题与关系档），图加载失败回退首字字牌。（2026-08-29，驱动指令/agent 执行）

- [DEC-036] **前端·开篇角色创建页（开场白页面）v1.0**：驱动四项裁定——① **前端直写变量**：DEC-005「不做向导、不伪造初始化」自本条起**限定为剧情侧口径**（N3 第 4 条「不做向导式自我介绍」本义），玩家本人在创角页的结构化填写是真实性最高的采集，不属向导式剧情、不属伪造初始化；创角页为整个前端**唯一写入口**，经 Mvu 正路直写 user 九字段＋`规则.初始化完成=true`（ZOD schema 既有预留字段首次获得消费方），一次写入后页面不再出现（幂等由实卡部署壳控制，随组装门）；渲染层（djg-render）仍只读，「前端只读」纪律修订为「**渲染层只读**」（变量映射 v1.2 §7）；② 住址=**下拉+自由补充**（七项下拉与城市层 C1-C5 词条名一致，供 INC-3 初遇路由五轴取数；选项零角色/轴提示；补充文本以 `·` 拼接）；③ 年龄**必填 18~99 整数**（NSFW 内容边界——避免年龄「未知」被剧情演绎为未成年，与 DEC-007/清寒 18 岁裁定同向；写入为字符串，schema text 兜底）；④ 开场背景**静态**——实现调整待追认：kv-pc.jpg 实为原神防沉迷公告文字页背景不可用，实景照片气质与 DEC-028「质感与原神明显区别」原则冲突，改**雾青夜空渐变＋city-carousel 城市剪影天际线**（页底装饰带，opacity .42＋雾青滤镜）。落盘（frontend/ 本地不入库）：`开场白-创角页.html`（三段式：开场叙事〔仅起源层 confirmed 公开信息：群名「但为君故」/活跃成员构成/主控 2026 秋最新入群＝活跃名单第 6 位〕→活跃成员栏〔雾态纪律：五人只网名+一号位 DC 头像＋「你的 ID 待创建」虚位卡〕→创角表单〔九字段，仅 DC 网名/年龄必填，留空记「未知」/「无」〕＋写入预览〔提交前整卡 JSON 确认，与「帮填只出预览」同构〕＋完成态摘要）＋ `djg-create.js`（纯逻辑层：校验+payload 组装，零依赖不触宿主 API，实卡复用；Mvu 写入壳随组装门）＋ `变量映射-v1.md` 升 v1.2（新增 §7 创角写入契约：DEC-005 补注记/写入口唯一性/九字段映射表/载荷结构/迷雾纪律自查）。实开校验通过：成员栏顺序（修 insertBefore 参照节点取到空白文本节点的缺陷）＋5/5 头像加载、年龄边界（17/100 拒绝、18 通过）、JSON 预览逐字段核对（年龄字符串化/住址拼接/未知兜底/初始化完成=true）、提交→完成态摘要正确、390px 窄屏（单列表单/两列成员栏/长网名 ellipsis）。边界同 DEC-028：本地设计稿不入库；实卡部署（酒馆助手壳+Mvu 写入）随组装门。页脚致谢（驱动追加指令，同日）：credits 两行（感谢 但为君故:只需等待线上抄咕委员会 提供人设／感谢玩家·投稿引导「大家都可以将自己的人设加入到南江市之中」）＋「但为君故，沉吟至今」雾青落款行（两侧细线呼应开场），文案按驱动原文未改。（2026-08-29，驱动四项裁定/agent 执行）

- [DEC-037] **组装门执行（用户指令：产出一个真正能导入酒馆的角色卡，并检阅预设适配）**：成品卡《在DC频道里口嗨的群友就住我隔壁？！》v1.0 = chara_card_v3 **双格式**（`输出/…v1.0.json` 653KB ＋ `…v1.0.png` 1.56MB，PNG 写入 chara[base64]+ccv3[原文] 双 tEXt 块，回读校验 `ccv3===chara===json` 一致；`输出/` 含内嵌头像立绘，按裁定④不入库）。组装器 `组装/`（build_card.mjs + make_images.ps1 + scripts/ + verify/spot_check + 导入与游玩说明.md，全部入库）。**卡内容**：① 世界书 48 条＝协议层 5（N0-N3 constant order 210-207；N4 constant **disable 默认 true** order 206）＋起源层 1（205）＋城市层 6（C0 200 constant；C1-C5 绿灯 195-191）＋场所层 6（U0 190；U1-U5 189-185，全绿灯）＋角色层 26（主词条 pos1 order 190/188/186/184/182；秘密词条 21 条 **默认关、同主词条 keys**、order 181-161 与源稿一致）＋变量层 4（`[initvar]` **enabled=false**——MVU v1.0.1 源码核实读 initvar 不查 enabled，禁用避免原始 YAML 进剧情提示词；`[mvu_update]`×3 constant order 215/214/213＝更新规则/输出格式/变量列表，高于协议层段位贴近聊天；内容按 DEC-024 四件 txt 原样照抄）；② 酒馆助手脚本 4 件＝MVU 加载器＋ZOD（DEC-024 照抄不动，单镜像；契约④双区域镜像序维持"后续增强项"）＋ **R2 迷雾门卫**（新实现：监听 VARIABLE_UPDATE_ENDED/INITIALIZED/chat_changed，任一知情者翻转即解锁秘密词条[DEC-025 口径，Y.逃兵→剧本映射]；MOD-A 按 **|好感度|≥50** 推导[DEC-033 阶梯校准，N4 源稿{亲近,恋人}系旧口径]；MOD-B 任一任务=进行中；幂等全量推导、新聊天自动复位、fail-soft）＋ **雾青前端渲染**（新实现：最新 AI 楼层五人状态栏＋右下角◇群像 overlay[跳板/档案/双头像位揭晓切换]，只读 stat_data，CSS 全作用域限定，parent.document 范式与预设悬浮窗同源）；③ 正则 3 条＝R3 渲染隐藏＋R3 promptOnly 历史剥离（UpdateVariable）＋content 标签显示隐藏（备用 disabled）；④ 开场白＝创角页实装（```html 代码块包裹[酒馆助手 isFrontend 约定：源码核实含 `<body` 即 iframe 渲染]、djg-create 内联、Mvu 写入壳[getCurrentMessageId+getMvuData/replaceMvuData]、幂等完成态、「进入频道」=triggerSlash /send+/trigger、剔除原神 cityline 背景依赖、头像降采样 data URI 内嵌）＋备选纯文本开场白 1 条（剧情链采集口径，DEC-005/205）；⑤ 头像/立绘降采样内嵌（256px 头像/576px 立绘 JPEG，源 20MB→卡内 ~300KB data URI，仅存本地不入库）。**预设适配检阅**（用户要求第二项）：适配版 prompt_order 基线与协议层 §6 核对单全对位（通用思维链/卡3.7f/备忘关/NSFW 标准/龙族关/同人关/台词匀/文风都市白描/防情感廉价开）；预设正则不识 `<UpdateVariable>`→R3 必要性证实并已随卡补齐；预设无 MVU 加载器→卡内脚本为唯一来源，无双重加载；MVU-offline v1.0.1 源码核实：`<Analysis>`/`<JSONPatch>` 标签解析兼容、`[mvu_plot]`/`[mvu_update]` 原生双模型路由、酒馆助手版本门槛 4.8.4。**ACC-005 离线校验通过**（约 200 断言：48 条结构/事实 ID 注册表↔秘密词条↔R2 表三方一致 21 格/initvar 105 迷雾单元 21-16-17-17-17/schema↔initvar 一致/更新规则纪律关键词/开场白无伪造初始化/三脚本编译通过/常驻词条 8406 字预算）。成品与图像构建产物在 `输出/`（gitignore 排除）；待 ACC-006 真实运行时验收（驱动另约）。改动后重建：`组装/make_images.ps1` → `组装/build_card.mjs`。**同日增补**：世界书单独导出器 `组装/export_worldbook.mjs` → `输出/但为君故·世界书 v1.0.json`（60KB，48 条，酒馆原生 entries 字典格式[服务端 import 硬性要求含 entries 键；字段映射照抄 ST release convertCharacterBook+newWorldInfoEntryDefinition 源码]；世界书面板可直接导入；不含脚本/正则/创角页——完整玩法仍需角色卡本体）。（2026-08-29，用户指令/agent 执行；四项组装门校准[N4 阶梯口径/initvar 禁用/变量层 order/R2 表]均随本条登记待追认）
...
```twa-acceptance
{
  ...
  "items": [
    ...
    },
    {
      ...
      "gate": "automation",
      "evidence": "2026-08-29 组装门通过（DEC-037）：build_card.mjs 内嵌断言约 200 项全过——48 词条结构/事实ID 注册表↔秘密词条↔R2 表三方一致（21 格）/initvar 105 迷雾单元（21+16+17×4）/schema↔initvar 一致/更新规则纪律关键词/开场白无伪造初始化/三脚本编译/常驻 8406 字预算；PNG chara+ccv3 双块回读与 JSON 逐字节一致",
      "status": "passed",
      "date": "2026-08-29",
      "actor": "automation"
    },
...
**ACC-006 真实运行时验收（驱动另约）**：导入成品卡（`输出/…v1.0.png/.json`，DEC-037）→ 开新聊天验证 开局初始化→创角页写入→变量更新→R2 门控生效→雾青渲染链 全程无报错。此前开放项：DEC-027 三档位提案、DEC-036 背景方案（天际线）、DEC-037 四项组装门校准的追认；DEC-202 城区修订 / DEC-203 任务层 / DEC-204 彩蛋层 / DEC-205 备选开场与起源层创始人——补齐后重跑 `组装/build_card.mjs` 重建卡即可，无需改组装器。
